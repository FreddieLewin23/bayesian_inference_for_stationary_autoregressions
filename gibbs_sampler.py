...
if __name__ == '__main__':
    ...
    cfg = GibbsConfig(
        p=7, # found by minimising AIC over p between 1 and 12 in the check_order_stationarity.py script
        n_iter=8000,
        burn=2000,
        thin=5,
        add_intercept=False,  # keep False if demean=True
        a0=2.0, b0_ig=1.0,
        enforce_stationarity=True,
        rng_seed=42
    )
    res = gibbs_ar(ts, cfg)
    post_phi = res.beta_samples.mean(axis=0)  # if no intercept, these are phi1, ... ,phip
    phi_ci = np.quantile(res.beta_samples, [0.025, 0.975], axis=0)
    post_sigma2 = res.sigma2_samples.mean()

    print("Posterior mean phi:", np.round(post_phi, 4))
    print("95% CI for phis rows:\n", np.round(phi_ci.T, 4))
    print("Posterior mean sigma^2:", post_sigma2)
    print("Stationary draw acceptance rate:", res.accepted_stationary_rate)

    # I WANT TO CHECK MY SAMPLER AGAINST THE NUTS SAMPLER USED IN PYMC
