functions {
  // Convert PACF (partial autocorrelations) to AR coefficients via Levinson recursion.
  vector pacf_to_ar(vector kappa) {
    int p = num_elements(kappa);
    vector[p] phi;           // final AR coefficients
    vector[p] phi_prev;      // workspace
    vector[p] phi_curr;      // workspace

    // m = 1
    phi_prev[1] = kappa[1];
    for (m in 2:p) {
      // start with previous and update
      for (i in 1:(m-1)) {
...
;
      ...
      // copy back into phi_prev (only first m entries are defined)
      for (i in 1:m) {
        phi_prev[i] = phi_curr[i];
      }
    }
    // output
    for (i in 1:p) phi[i] = phi_prev[i];
    return phi;
  }
}
data {
